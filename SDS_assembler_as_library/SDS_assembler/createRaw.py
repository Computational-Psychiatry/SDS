...
import pandas as pd
import logging
from SDS_assembler.utility_fuctions import (
    csv_to_json_converter,
    yml_to_json_converter,
    mandatory_fields_json_checker,
    file_name_creator,
...
class CreateRaw:
    def __init__(self, raw_folder_name):
        # Constructor for CreateRaw class
        self.raw_folder_name = raw_folder_name
        self.logger = logging.getLogger(__name__)

    def file_system_copy_and_organizer(self):
        try:
            # Copy files to the specified folder and organize them
            video_full_path_to_copy = self.fileextension
            filename_info_with_extension = self.fileextension.split(os.sep)[-1]
            ...
            final_copied_path_location_name = os.path.join(self.folder_content, filename_info_with_extension)
            ...
            self.logger.error(f"Error in file_system_copy_and_organizer: {str(e)}")
    ...
    def file_name_modifier(self, fields_filename_mandatory_info_correct):
        # Modify file names based on specified criteria
    ...
    def mandatory_field_info_checker_info_extractor(self, all_csv_file_extensions, all_json_file_extensions, all_yaml_file_extensions):
        # Extract and check mandatory field information from files
        status = None
        file_creators_list = []
        for csv_file_extensions in all_csv_file_extensions:
            field_values_csv, status = mandatory_fields_json_checker(
                csv_file_extensions[0], csv_file_extensions[1], csv_file_extensions[2]
            )
            if field_values_csv:
                file_creators_list.append(field_values_csv)
        for json_file_extensions in all_json_file_extensions:
            field_values_json, status = mandatory_fields_json_checker(
                json_file_extensions[0], json_file_extensions[1], json_file_extensions[2]
            )
            if field_values_json:
                file_creators_list.append(field_values_json)
        for yaml_file_extensions in all_yaml_file_extensions:
            field_values_yaml, status = mandatory_fields_json_checker(
                yaml_file_extensions[0], yaml_file_extensions[1], yaml_file_extensions[2]
            )
            if field_values_yaml:
                file_creators_list.append(field_values_yaml)
        return file_creators_list, status

    def file_extension_check(self, df):
        # Check file extensions and process files
        if "filename_extensions" in list(df.columns):
            ...
            all_csv_file_extensions = []
            all_json_file_extensions = []
            all_yaml_file_extensions = []
            ...
            for filename in all_location_extractor:
                ...
                if filename.endswith((".mp4", ".mkv", ".avi")):
                    ...
                    if filename_other_csv in all_extension_extractor:
                        json_file_from_csv = csv_to_json_converter(filename_other_csv)
                        all_csv_file_extensions.append([json_file_from_csv[0], filename_other_csv, filename_video_info, filename_other_csv])
                    elif filename_other_json in all_extension_extractor:
                        with open(filename_other_json, 'r') as f:
                            json_data = json.load(f)
                        all_json_file_extensions.append([json_data, filename_other_json, filename_video_info, filename_other_json])
                    elif filename_other_yml in all_extension_extractor:
                        json_from_yml = yml_to_json_converter(filename_other_yml)
                        all_yaml_file_extensions.append([json_from_yml, filename_other_yml, filename_video_info, filename_other_yml])
                    else:
                        self.logger.warning(f"Skipping file without an extension: {filename}")
                        status = False
        else:
            self.logger.error("Column 'filename_extensions' not found in DataFrame")
            status = False
        return all_csv_file_extensions, all_json_file_extensions, all_yaml_file_extensions, status

    def folder_structure_source_checker(self):
        # Check for the presence of specific files in the source folder
        files_all = []
        ...
        if "dataset_description.json" in files_all:
            self.logger.info("Success: 'dataset_description.json' present in " + str(self.source_folder_name))
            self.full_data_description_path = os.path.join(self.source_folder_name, "dataset_description.json")
        else:
            self.logger.error("Mandatory File Missing: 'dataset_description.json' in " + str(self.source_folder_name))
        if "README" in files_all:
            self.logger.info("Success: 'README' present in " + str(self.source_folder_name))
            self.README = os.path.join(self.source_folder_name, "README")
        else:
            self.logger.error("Mandatory File Missing: 'README' in " + str(self.source_folder_name))

def createRaw(df, raw_folder_name):
    csvCreator = CreateRaw(raw_folder_name)
    all_csv_file_extenders, all_json_file_extenders, all_yaml_file_extenders, status_file_extension_source = csvCreator.file_extension_check(
        df)
    # csvCreator.folder_structure_source_checker()
    files_field_names_mandatory_correct, status_json_checker = csvCreator.mandatory_field_info_checker_info_extractor(
        all_csv_file_extenders, all_json_file_extenders, all_yaml_file_extenders)
...
# Uncomment the method below and the related code if needed
#     def csv_creator(self, all_csv_file_extensions, all_json_file_extensions, all_yaml_file_extensions, write_location):
#         data_list = []
#         for csv_file_extensions in all_csv_file_extensions:
#             dict_csv_extender = csv_file_extensions[0]
#             dict_csv_extender["filename"] = csv_file_extensions[2]
#             dict_csv_extender["filename_extended"] = csv_file_extensions[3]
#             data_list.append(dict_csv_extender)
#         for json_file_extensions in all_json_file_extensions:
#             dict_json_extender = json_file_extensions[0]
#             dict_json_extender["filename"] = json_file_extensions[2]
#             dict_json_extender["filename_extended"] = json_file_extensions[3]
#             data_list.append(dict_json_extender)
#         for yaml_file_extensions in all_yaml_file_extensions:
#             dict_yml_extender = yaml_file_extensions[0]
#             dict_yml_extender["filename"] = yaml_file_extensions[2]
#             dict_yml_extender["filename_extended"] = yaml_file_extensions[3]
#             data_list.append(dict_yml_extender)
#         df = pd.DataFrame.from_dict(data_list, orient='columns')
#         print(df.head())
#         df.insert(0, column="filename
